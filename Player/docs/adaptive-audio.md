...
manifest contract (section 5), and the rule that React Native only observes the
...
Weight in the aggregate: **0.2**. The input scans for the standard Bluetooth LE
Heart Rate service (`0x180D`), ignores every device Android does not already
report as bonded, connects without presenting an in-app picker, and subscribes
to Heart Rate Measurement (`0x2A37`) notifications. Both the profile's 8-bit
and 16-bit BPM encodings are accepted.

Heart rate is normalized linearly from 40-200 BPM onto 0-9 and clamped at both
ends. Until the first valid notification arrives, and again immediately after
a disconnect, the input is unavailable; a disconnect resumes scanning so a
strap that wakes or reconnects mid-song begins driving playback without a
restart.

On Android 12 and newer, `BLUETOOTH_SCAN` and `BLUETOOTH_CONNECT` are requested
together only when `play()` is first called for `heartRate`, `intensity`, or an
unknown dimension that resolves as intensity. The attempt is persisted. A
denial is never re-prompted and does not stop playback; it is exposed only as
the same unavailable reading the existing badge already reports. On older
Android releases the corresponding manifest permissions are install-time.
Android 6-11 expose BLE scanning through the fine-location runtime permission;
the host requests that platform prerequisite at the same lazy playback point,
limits it to those OS versions in the manifest, and never uses scan data for
location.
...
## 6. Queue invalidation on track switch
