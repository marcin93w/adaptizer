...
| `onDimensionChanged` | `{ dimension: string; value: number; volume: number; movementSpeed: number; heartRate: number }` | Fired whenever the current song's dimension is re-resolved - any input changing, a new reading or an availability flip alike - see [`adaptive-audio.md`](adaptive-audio.md) sections 1-4. `dimension` is the resolved dimension the module used (the current song's, one of the four contract names); `value` is the resolved value `0-9` that actually drives track selection. `volume`, `movementSpeed` and `heartRate` are the per-input diagnostic readings, each an integer `0-9` or `-1` when that input is unavailable (the same `-1` sentinel `onProgress.durationMs` uses) - so the same fields report what each input read *and* which inputs are unavailable. Inputs never fabricate a value, so a non-`-1` reading is always a real measurement. Consumers render `value` and treat the per-input fields as diagnostics only; adding or removing an input changes the bridge contract. Presentation only; it implies no JS-side decision-making. |
