...
It installs side by side with the legacy Android app under `app/` and does not
depend on or build against it.

Reference docs:

- [`../docs/adaptive-audio.md`](../docs/adaptive-audio.md) - how intensity and
  track selection work
...
- `applicationId` / `namespace`: **`com.adaptizerplayer.rn`** - a
  **temporary** ID (not the production `com.adaptizerplayer` ID) so this
  app installs side by side with the legacy app. It must change to the
  production identity, with production signing and a correct
