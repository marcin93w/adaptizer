...
| c | Control-view listeners must not grow across rerenders. `control-summary.tsx` and the curve editor register in effects and unregister when the control changes or unmounts. Adaptizer listener lifecycle remains a separate concern. | **Fixed, both halves.** `Adaptizer.dispose()` unregisters and drops pending sends; the configurator calls it when the memo yields a new adaptizer. The debounce moved off the control and onto the adaptizer - see (e). Pinned by T3. |
