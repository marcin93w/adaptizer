...
- **DOM: jsdom**, not happy-dom. Slower, and it lacks the `ResizeObserver` react-range wants, but `@testing-library/react` and `user-event` are developed against it, and the export dialog leans on `<input type="number">` and `localStorage` semantics. The missing observer costs five lines once.
...
| | File | Covers |
| --- | --- | --- |
| **T1** | `domain/__tests__/control-transform.test.ts` | The input to CC transform, ported from the golden table in `Instrument/adaptizer.test.py` - the only logic in the repo with an independent oracle. 30 of 38 rows port (15 LINEAR + 15 REVERSED_LINEAR); the 8 BINARY rows have no TypeScript equivalent. **Column-order warning:** the Python parameter names are misleading - the table is really `(midiMin, midiMax, inputType, inputMin, inputMax, transform, input, expected)`, per `adaptizer.py:34`. Plus the ten default-control values `0, 14, 28, 42, 56, 71, 85, 99, 113, 127`, which *are* the ten exported DASH tracks. |
| **T2** | `domain/__tests__/project-dto.test.ts` | `.adz` round-trip asserted **through the transform** - every control produces the same CC value for every input 0..9 after save and reopen. Plus the DTO key names as a compatibility contract (the one place where structure *is* behavior). |
| **T3** | `domain/__tests__/adaptizer.test.ts` | Debounce and send rules, asserted against the fake port's message log: the DAW is in sync on load; a new control is auditionable at once; **dragging a slider does not flood the port** (one message, final value, 300 ms after the last change); controls debounce independently; **turning the knob is instant**; selecting a control re-announces it so the DAW can learn the CC. |
...
- **`react-range`'s drag geometry.** Pointer maths read `getBoundingClientRect`, which jsdom reports as all zeros; any drag test asserts against fabricated geometry - testing your stub, not the app - and it's a third-party library's job.
...
- **`Control` getters/setters in isolation.** `expect(control.inputMin).toBe(5)` after assigning 5 tests the language. The interesting consequence is T3.
...
| c | Unbounded listener growth - `linear-control.tsx:26` registers in the render body, and `Control` has no unsubscribe anywhere. `configurator.tsx` builds **two** Adaptizers on mount, both stashing their timer on `control['_debounceTimeout']` - the same property on the same object - so one message survives **by accident**. Fix: return an unsubscribe (precedent: `preload.ts:38`) and move registration into an effect. The test that states the rule ("two Adaptizers over one project each send their own message") lands with the fix, not before it. | **Fix (batch 2)** |
...
| h | `inputMin === inputMax` gave `0 / 0`, so `NaN` reached `midiOutput.send([0xB1, cc, NaN])`, and Web MIDI **throws a `TypeError` on non-integer data** - the live audition died until restart. A collapsed range is a legitimate setting ("when intensity is exactly 5, send this"), so it now reads as full scale: `midiMax` for linear, `midiMin` for reversed. Throwing was considered and rejected - every caller is a render body or an unhandled timer callback, so an exception would turn a quiet audition into a blank window. Batch 2 should also give the input slider a minimum thumb distance. | **Fixed (batch 1)** |
